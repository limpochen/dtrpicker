...
class HeaderBarCell extends Cell {
   ...
   */
  _sortedColors() {
    var cells = this.picker._headerCells;
    if (!cells || cells.length === 0) return [];
    var fallback = this.picker.options.cellColor;
    var arr = cells.map(function (c, idx) {
  ...
  render() {
    ...
    var gradId = this.picker._instanceId + '-hdr-grad';
    var oldDef = this.picker.svg.querySelector('#' + gradId);
    ...
    var sortedColors = this._sortedColors();
    if (sortedColors.length < 2) return;

    var defs = this.picker.svg.querySelector('defs');
    if (!defs) {
      defs = document.createElementNS(this.g.svgNS, 'defs');
      this.picker.svg.insertBefore(defs, this.picker.svg.firstChild);
    }
    var grad = document.createElementNS(this.g.svgNS, 'linearGradient');
    ...
    sortedColors.forEach(function (color, idx) {
      var pct = (idx / (sortedColors.length - 1)) * 100;
